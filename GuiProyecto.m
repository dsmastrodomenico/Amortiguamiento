...
% Last Modified by GUIDE v2.5 15-Nov-2018 13:17:03
...
set(handles.estab,'string',funciones.estabilidad(polos));

amor=funciones.amortiguamiento(g);
set(handles.amor,'string',amor);

sPas=funciones.sobrepaso(g);
set(handles.sPas,'string',sPas);

tAs=funciones.tAsentamiento(g);
set(handles.tAs,'string',tAs);
...
kEst=funciones.estabilidadK(num,den);
set(handles.kEst,'string',kEst);

[ram,asint,inter,ang]=funciones.LGR(polos,ceros);
set(handles.ramas,'string',ram);
set(handles.asint,'string',asint);
set(handles.intersec,'string',inter);
set(handles.ang,'string',num2str(ang));

rP=funciones.pRuptura(num,den);
set(handles.ramas,'string',evalc('rP'));
